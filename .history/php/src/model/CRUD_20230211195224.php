...
class db{
    ...
    public function getDataOfList($tablename, $list_name){
        $this->tablename = $tablename;
        $this->list_name = $list_name;
        $query = "SELECT l.name AS list_name, 
        li.quantity, 
        COALESCE(p.product_name, pi.item_name) AS item_name, 
        COALESCE(p.product_description, '') AS item_description, 
        COALESCE(p.product_image, '') AS item_image 
        FROM list AS l 
        JOIN list_items AS li ON l.id = li.list_id 
        LEFT JOIN products AS p ON li.id_product = p.id_product 
        LEFT JOIN private_items AS pi ON li.item_key = pi.item_key

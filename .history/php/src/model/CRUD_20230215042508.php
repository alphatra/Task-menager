...
class db{
    ...
    public function getDataOfList(){
        $sql = "SELECT JSON_OBJECT( 'list_name', l.name, 'items', JSON_ARRAYAGG( JSON_OBJECT( 'item_name', p.product_name, 'quantity', il.quantity ) ) ) FROM list l JOIN list_items il ON l.id = il.list_id JOIN products p ON il.id_product = p.id_product GROUP BY l.id;
        ";
        $result = mysqli_sql($this->conn, $sql);
        $result = mysqli_fetch_all($result, MYSQLI_ASSOC);

...
class db{
    ...
    public function getDataOfList($query = null){
        ...
        if(isset($this->query) && $this->query != ''){
            $arg = "WHERE product_name LIKE '$this->search%' OR l.created_date LIKE '$this->query%'";
        }
        $sql = "SELECT l.name, l.created_date, l.end_date, l.priority, COALESCE(p.product_name, pi.item_name) AS product_name, li.quantity 
                FROM list l 
    ...
    public function deleteList($list_id) {
        ...
        $query = "DELETE FROM list WHERE id = $this->list_id;DELETE FROM list_items WHERE list_id = $this->list_id";
    ...
    */
    ...
    public function createListItems($qty,$list_name,$id_product, $item_key=null){
        $id = $this->getMaxId('list_items');
        $this->qty = $qty;
        $this->list_name = $list_name;
        if(!is_numeric($this->list_nam)){
        ...
        $sql = "INSERT INTO list_items (id, quantity, list_id, id_product, $arg) VALUES ('{$id}', '{$this->qty}', '{$this->list_name}', '{$this->id_product}','{$arg2}')";

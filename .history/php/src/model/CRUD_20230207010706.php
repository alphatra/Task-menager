...
class db{
    ...
    */
    ...
    public function createList($list_name, $created_date, $end_date){
        ...
        $_max_id = mysqli_query($this->conn, $query);
        $row = mysqli_fetch_assoc($_max_id);
        echo $row;
        $row = $row+1;
        $query = "INSERT INTO list (id, list_name, created_date, end_date) VALUES ('{$row}','{$list_name}','{$created_date}','{$end_date}')";
        mysqli_query($this->conn, $query);
    }
}
?>
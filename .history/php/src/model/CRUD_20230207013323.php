...
class db{
    ...
    */
    ...
    public function getMaxId(){
        $sql = "SELECT MAX( id ) FROM list AS Max_Id";
        $result = mysqli_sql($this->conn, $sql);
        $result = mysqli_fetch_all($result, MYSQLI_ASSOC);
        
        return json_encode($result);
    }
    public function createList($list_name, $created_date, $end_date){
        ...
        $sql = "INSERT INTO list (id, list_name, created_date, end_date) VALUES ('{$next_id}','{$list_name}','{$created_date}','{$end_date}')";
        return mysqli_sql($this->conn, $sql);
    }
}
?>
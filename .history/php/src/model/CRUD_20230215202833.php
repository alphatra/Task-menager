...
class db{
    ...
    public function getDataOfList(){
        $sql = "SELECT l.name, p.product_name, li.quantity FROM list l LEFT JOIN list_items li ON l.id = li.list_id LEFT JOIN products p ON li.id_product = p.id_product ORDER BY l.name;";
        $result = mysqli_sql($this->conn, $sql);
        $result = mysqli_fetch_all($result, MYSQLI_ASSOC);

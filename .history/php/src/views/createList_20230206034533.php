...
<head>
    ...
    let searchedProduct = (parseInt(_id)) =>{
        return _dataItems.find(e => e.id_product === _id).map(e => e.product_name)
    } 
        
    let generateContent = (list) => {
        let content = $('#list_elements')
        content.empty()
        list.map((item) => { console.log(searchedProduct(parseInt(item)));})
        let listItems = list.map((item) => { return $(`<h5>${searchedProduct(item)}</h5>`)})
        content.append(listItems)
    }

    $(document).on("click", ".btn", function() {
        add_products(this.name);
        generateContent(list.products)
        console.log(list);
    });

$(document).ready(function() {
    ...
    $(document).on("click", "Button[data-property=Edit]", function() {
        ...
        $.each(Data[productId].products, function(index, product) {
            ...
            let input_quantity = $('<input>').attr('type', 'text');
            ...
            $('<td>').append(input_quantity.attr('value', product.quantity).attr('org-quantity', 'quantity')).appendTo(row);
        ...
        var addButton = $('<button>').text('Dodaj').attr('data-property', 'Add').attr('data-product-id', last_i+1).attr('data-list-name', Data[productId].name);
        var row = $('<tr>');

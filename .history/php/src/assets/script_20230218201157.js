$(document).ready(function() {
    ...
    const groupByList = (data) => {
        const result = data.reduce((acc, { name, product_name, quantity, created_date, end_date, priority, id }) => ({
        ...acc,
        [name]: {
        name,
        created_date,
        end_date,
        priority,
        products: [...(acc[name]?.products || []), { product_name, quantity,id }],
        },
        }), {});
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $.each(_output, function(index, list) {
                    ...
                    let deleteButton = $('<button>').addClass('').attr('data-property', 'Delete').attr('data-listid', list.name).text('Usuń');
  ...
  // utworzenie wiersza nagłówka
        ...
        $.each(_output[productId].products, function(index, product) {
            ...
            var input_quantity = $('<input>').attr('type', 'number').attr('org-name', 'quantity');
            ...
            $('<td>').append(input_quantity.attr('value',product.quantity)).attr('index',index).appendTo(row);
    ...
    $(document).on("click", "Button[data-property=Add]", function() {
        ...
        $.ajax({
            ...
            success: function(result) {
                ...
                let row = $('<tr>');
                let td = $('<td>');
                td.text(id+1).appendTo(row)
                td.text(name).appendTo(row)
                td.text(qty).appendTo(row)
                td.append(deleteButton,setButton).appendTo(row)
                row.appendTo(table);
            }
        });
    }); 

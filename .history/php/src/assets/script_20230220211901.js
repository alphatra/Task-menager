$(document).ready(function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $.each(data, function(index, list) {
                    ...
                    let deleteButton = $('<button>').addClass('').attr('data-property', 'Delete').attr('data-listid', list.idl).text('Usuń');
    ...
    $(document).on("click", "Button[data-property=Edit]", function() {
        ...
        let input_quantity = $('<input>').attr('type', 'number').attr('name','quantity').attr('pattern','[0-9]+').attr('min', '1').attr('max', '100').attr('inputmode', 'numeric');
        let addButton = $('<button>').text('Dodaj').attr('data-property', 'Add').attr('data-product-id', rows.length).attr('data-list-name', Data[productId].name);
        let newRow = $('<tr>');
        $('<td>').text(rows.length + 1).appendTo(newRow);

$(document).ready(function() {
  ...
  // utworzenie wiersza nagłówka
        ...
        let last_i
        console.log(_output[productId].name);
        ...
        var addButton = $('<button>').text('Add').attr('data-property', 'Add').attr('data-product-id', last_i+1).attr('data-list-id', productId);
    ...
    $(document).on("click", "Button[data-property=Add]", function() {
        ...
        console.log(list_id, id,name,qty);
        $.ajax({
            url: "addTo_list.php",
            type: "post",
            data: {list:list_id, id: id, product_name:name, quantity:qty},
            success: function(result) {
                // wykonaj kod po usunięciu listy
            }
        });

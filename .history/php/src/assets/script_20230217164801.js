$(document).ready(function() {
  ...
  // utworzenie wiersza nagłówka
        ...
        var deleteButton = $('<button>').text('X').attr('data-property', 'Delete').attr('data-product-id', last_i+1).attr('data-list-name', _output[productId].name);
        var row = $('<tr>');
        $('<td>').text(last_i+1).appendTo(row);
        $('<td>').append(input_name).appendTo(row);
        $('<td>').append(input_quantity).appendTo(row)
        $('<td>').append(addButton,deleteButton).appendTo(row)
        table.append(row);

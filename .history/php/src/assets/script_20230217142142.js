$(document).ready(function() {
  ...
  // utworzenie wiersza nagłówka
        ...
        var input_quantity = $('<input>').attr('type', 'text').attr('name', 'quantity').attr('placeholder', 'qty');
        var addButton = $('<button>').text('Add').attr('data-property', 'Add').attr('data-product-id', last_i+1).attr('data-product-name', input_name.val()).attr('data-product-quantity', input_quantity.val());
        $('<td>').text(last_i+1).appendTo(table);
        $('<td>').append(input_name).appendTo(table);
    ...
    $(document).on("click", "Button[data-property=Add]", function() {
        const id = $(this).data('data-product-id');
        const name = $(this).data('data-product-name');
        const qty = $(this).data('data-product-quantity');
        console.log($(this));

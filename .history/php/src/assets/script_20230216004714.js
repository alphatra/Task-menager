    ...
    let loadData = (param) => {
        $.ajax({
            ...
            success: function(data) {
                ...
                _data.forEach((group) => {
                    // utworzenie elementu HTML dla grupy
                    let groupElement = $(`<div class="group"><h2>${group.name}</h2></div>`);
                    // pętla po każdym elemencie w grupie
                    group.items.forEach((item) => {
                        // utworzenie elementu HTML dla produktu
                        let itemElement = $(`<div class="item"><h3>${item.product_name}</h3><p>Quantity: ${item.quantity}</p></div>`);
                        // dodanie elementu produktu do elementu grupy
                        groupElement.append(itemElement);
                    });
                    // dodanie elementu grupy do głównego kontenera
